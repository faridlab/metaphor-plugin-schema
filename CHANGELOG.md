...
- **`metaphor schema tenancy` — composition-installed tenancy (ADR-0029).** Modules ship no
  tenancy; the composing backend-service owns a `tenancy.yaml` descriptor at its root, and this
  verb turns it into a service-owned decorator migration chain: `ADD COLUMN IF NOT EXISTS
  org_unit_id` with a company_id backfill guarded by runtime introspection, NOT NULL, a DEFAULT
  resolving `app.acting_unit_id` (unset/empty → NULL → loud NOT NULL violation), the
  entitlement-union RLS policy, the kind-guard trigger, per-unit unique indexes, and a
  deny-by-default event trigger over the scoped schemas (any later-created table there is
  locked — RLS armed with a false/false policy — until the descriptor covers it). The chain is
  idempotent and converges on fresh, adopted, and already-decorated databases. Modes:
  `--output <dir>` (default `migrations/`) writes the pair; `--preview` prints it; `--check`
  (database-feature build) introspects a live database against the descriptor — column
  presence/nullability/default, policy, RLS enable+force, kind-guard trigger, uniques, event
  trigger — and exits non-zero on any gap, so it can gate CI. Table references resolve through
  the workspace: a module missing from `metaphor.yaml` or a table the module does not define
  fails at emission, not against a live database. The emitted header deliberately avoids the
  `-- Generated by metaphor-schema` marker so `schema generate --force` cannot sweep the pair;
  listing it under `user_owned:` remains required regardless.

### Changed

- **An absent `company_fence:` declaration is legal and warning-free everywhere.** Under
  ADR-0029 modules are tenant-agnostic — undeclared is the intended end state, and the
  module-level declaration survives only until each module's strip release. The hard gates in
  `schema validate` and `schema validate-workspace` are removed; `generate`'s warning is
  removed; `validate-workspace` now prints a sweep-progress note counting the modules still
  DECLARING (each retires with its strip release), and a future train inverts that note to a
  failure once the sweep completes. Declared-fence audits (declaration with no effect,
  `shared_blank` with a dead NULL arm) still warn while a declaration exists.

### Fixed

- **The `database` feature compiles again.** Live introspection's `TableSnapshot` initializer
  missed the three org fields added for ADR-0028 (`org_scoped`, `org_root_shared`, `org_fence`),
  so `cargo check --features database` failed — invisible to CI because the release workflow
  builds default features. The introspected side now sets all three neutral, matching the
  documented rule that fence posture is read only from the schema-derived snapshot.

## [0.14.0] — 2026-09-07

### Added

- **The org fence is declared in model yaml and generated end to end (ADR-0028).** A module
  declares `org_fence:` in `index.model.yaml`; models scope with `org_unit_id` fields, and the
  generator emits the entitlement-union RLS policy and the org-tree kind-guard trigger from the
  declaration. Undeclared or half-declared org scoping (column without declaration,
  declaration without column) is rejected at resolve time. *(Superseded in direction by
  ADR-0029 — the declaration site moves to the composing service's tenancy descriptor — but
  the emission machinery and the org RLS/kind-guard templates are what the tenancy decorator
  reuses.)*
- **Incremental migrations can re-key `company_id` → `org_unit_id`**, copying values and
  keeping partial-index predicates, with apply-order simulation refusing to emit a chain that
  references not-yet-created relations.
...
can no longer split a migration's `.up.sql` and `.down.sql` across two
timestamps, and a hand-written migration sharing a name can no longer be
silently reused (or clobbered) by the generator.

### Changed

- **Timestamp stabilization now keys on a migration's base name — the identity
  shared by its up/down pair — and assigns the pair one shared timestamp.**
  Previously the stabilizer matched on the per-direction suffix, so when a
  migration's `.up.sql` existed on disk but its `.down.sql` did not (an
  asymmetric pair), a regenerated down landed at `max+1` while the up was
  reused at its existing timestamp — splitting one logical migration across two
  timestamps, which the migration runner then applied as two. The pair now
  always lands on a single timestamp: reused when the generator already owns it,
  fresh otherwise. [`stabilize_migration_timestamps`](src/commands/schema/generate/stabilize.rs).

### Fixed

- **A hand-written migration sharing a base name is never reused.** The
  stabilizer reused any on-disk timestamp whose logical name matched, regardless
  of authorship. A hand-written migration (no `-- Generated by metaphor-schema`
  marker) that happened to share a name with a generated entity would have its
  timestamp reused and the file overwritten in place, silently clobbering
  hand-written SQL. Only **generator-authored** migrations (marker present) are
  now reused for stabilization; a hand-written collision gets a fresh `max+1`
  timestamp and the hand-written file is left untouched for the cleanup pass to
  preserve.

### Internal

- Replaced the two separate directory scans
  (`existing_timestamp_for_suffix` + `max_migration_timestamp`) with a single
  `build_migration_index` pass that records both the maximum timestamp and the
  generator-authored base names in one walk. New helpers `migration_base_name`
  and `migration_direction` extract the pair identity and the `.up.sql` /
  `.down.sql` suffix. Behaviour is unchanged for symmetric, generator-owned
  pairs.
