...
| `--force`, `-f` | flag | - | Overwrite existing files. Also bypasses the **migration identity dedup** check (re-emits a fresh-timestamp `<…>_create_<table>_table.{up,down}.sql` even when a same-identity migration already exists). A **timestamp stabilization** pass runs first under both plain `generate` and `--force`: existing migrations keep their on-disk timestamp (rewritten in place), new ones get a timestamp strictly greater than all existing ones, and duplicate-timestamp corruption is pinned to the minimum timestamp deterministically. The cleanup pass then sweeps stale `.up.sql` **and** their paired `.down.sql` files; hand-written migrations (no `-- Generated by metaphor-schema` header) and `user_owned` matches are preserved. |
