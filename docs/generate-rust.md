...
- `EntityRepoMeta::company_field()` override emitted whenever the model carries a `company_id` column that is not marked `@global`, pairing the generated Rust with the Postgres RLS fence for the same table (see [Company Scoping](schema-format.md#company-scoping)). Emission keys on the column's presence, so a new entity is fenced by default and `@global` is the explicit opt-out
...
per-direction suffix. One timestamp is chosen for the whole pair, so the `.up.sql`
and `.down.sql` can never split across two timestamps (a regenerated `.down.sql`
lands at the same timestamp as its `.up.sql` even when the down was missing on
disk — otherwise the runner would see one migration twice):

- If a **generator-authored** migration with the same base name already exists
  (its `.up.sql` carries the `-- Generated by metaphor-schema` marker), the pair
  is **rewritten in place at that on-disk timestamp** — its position in the
  applied ordering never shifts across regens.
- If it is genuinely new, it gets a fresh timestamp **strictly greater** than
  ...
  existing file or another new migration in the same run. Multiple new
  migrations get successive timestamps assigned in sorted base-name order, so
  the result is deterministic across runs.
- A **hand-written** migration (no marker) sharing the base name is **never
  reused** — the generated migration gets a fresh max+1 timestamp, and the
  hand-written file is left untouched for the cleanup pass to preserve.
- If the same base name was left on disk under multiple timestamps (legacy
