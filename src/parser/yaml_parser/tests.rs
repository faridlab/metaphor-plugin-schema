use super::helpers::{find_colon_outside_quotes, parse_type_string, split_attr_args};
use super::*;
...
fn test_split_attr_args_with_json_content() {
    ...
    assert!(
        result[0].contains("\"regular\":0"),
        "Should contain full JSON"
    );
    assert!(
        result[0].contains("\"kilat\":0"),
        "Should contain kilat key"
    );
}
...
fn test_find_colon_outside_quotes_nested() {
    ...
    assert_eq!(
        result2,
        Some(9),
        "Should find colon outside quoted strings, not inside"
    );
}
...
"#;
    ...
    metadata_fields.insert(
        "tags".to_string(),
        YamlField::Simple("string[]".to_string()),
    );
    shared_types.insert("Metadata".to_string(), metadata_fields);
