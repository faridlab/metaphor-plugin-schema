...
impl ContractsGenerator {
    ...
    /// Emit framework-free shared types used across generated layers
    /// (`shared/types/pagination.ts`), e.g. the generic `PaginatedResponse<T>`.
    fn generate_shared_types(&self, result: &mut DomainGenerationResult) -> Result<()> {
        let dir = self.config.output_dir.join("shared").join("types");
        let pagination = r#"/**
...
"#;
        self.write_index(dir.join("pagination.ts"), pagination.to_string(), result);
        self.write_index(
            dir.join("index.ts"),
            "// Shared types — generated by metaphor-webgen\nexport * from './pagination';\n".to_string(),
            result,
        );
        Ok(())
    }
