...
impl MobileGenerator {
    ...
    pub fn new(package_name: impl Into<String>) -> Result<Self> {
        ...
        // Register all templates + helpers
        Self::register_templates(&mut handlebars)?;

        // Generated code lives under a `<base>.generated` namespace, physically
        // `kotlin/<base-path>/generated/…`. Accept either the true base
        // (`com.bersihir`) or an already-suffixed value and normalize to exactly
        // one trailing `.generated` segment (package-agnostic — `<base>` may be
        // anything: `com.bersihir`, `id.startapp`, …).
        let raw = package_name.into();
        let base = raw.strip_suffix(".generated").unwrap_or(&raw);
        let package_name = format!("{}.generated", base);
    ...
    /// True base package (hand-written framework root), e.g. `com.bersihir`.
    /// Strips the trailing `.generated` segment from [`Self::package_name`].
    pub fn framework_package(&self) -> String {
        self.package_name
    ...
    fn register_templates(handlebars: &mut Handlebars) -> Result<()> {
        // Helper: `{{framework base_package}}` strips a trailing `.generated`
        // segment so generated files import hand-written framework base classes
        // (core.*, OfflineFirstRepository, domain.types.*, …) from the true base
        // package while everything else stays under `<base>.generated`.
        handlebars_helper!(framework: |s: str| {
            s.strip_suffix(".generated").unwrap_or(s).to_string()
        });
        handlebars.register_helper("framework", Box::new(framework));
    ...
    pub fn generate(
        ...
        let _ = std::fs::write(&codegen_yaml, CODEGEN_OWNERSHIP_MANIFEST);
...
/// Ownership manifest dropped at the root of the generated tree.
const CODEGEN_OWNERSHIP_MANIFEST: &str = r#"# metaphor.codegen.yaml — generated by metaphor-schema (generate:kotlin)
#
# Everything in this `generated/` subtree is OWNED BY THE GENERATOR and is
# overwritten on every regen — NEVER hand-edit it. Change the schema instead.
#
# Hand-written code lives OUTSIDE this tree (sibling packages under the base
# package, e.g. `<base>.core`, `<base>.infrastructure.di`) and customizes
# generated code via extension functions / subclasses / wrapper DTOs — never by
