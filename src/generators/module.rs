...
impl ModuleGenerator {
    ...
    fn generate_lib_rs(&self, schema: &ResolvedSchema) -> Result<String, GenerateError> {
        ...
        let model_names: std::collections::HashSet<&str> = schema
            .schema
            .models
            .iter()
            .map(|m| m.name.as_str())
        ...
        let hooks_with_computed = schema
            .schema
            .hooks
            .iter()
        ...
        for model in &schema.schema.models {
            ...
            if model_names.contains(service_name.as_str()) {
                ...
                writeln!(
                    output,
                    "pub use application::service::{0} as {1}AppService;",
        ...
        if schema.schema.authorization.is_some() {
            ...
            writeln!(
                output,
                "    RbacMiddleware, PermissionGuard, RequirePermission, rbac_layer,"
            )
            .unwrap();
            writeln!(output, "}};").unwrap();
            writeln!(output).unwrap();
        ...
        let hooks_with_validation = schema
            .schema
            .hooks
            .iter()
        ...
        let hooks_with_triggers = schema
            .schema
            .hooks
            .iter()
        ...
        for model in schema.schema.models.iter().filter(|m| handler_emitted(m)) {
            ...
            writeln!(
                output,
                "            .merge(create_{}_read_routes(self.{}_service.clone()))",
        ...
        for model in &schema.schema.models {
            ...
            writeln!(
                output,
                "        let {}_repository = Arc::new({}Repository::new(db_pool.clone()));",
                snake_name, name
            )
            .unwrap();
            writeln!(
                output,
                "        let {}_service = Arc::new({}::with_repository({}_repository.clone()));",
                snake_name, service_type, snake_name
            )
            .unwrap();
            writeln!(output).unwrap();
        }
    ...
    fn generate_routes(&self, schema: &ResolvedSchema) -> Result<String, GenerateError> {
        ...
        for model in schema.schema.models.iter().filter(|m| handler_emitted(m)) {
            ...
            writeln!(
                output,
                "    pub fn {}_routes(service: Arc<{}Service>) -> Router {{",
                snake_name, model.name
            )
            .unwrap();
    ...
    fn generate_grpc(&self, schema: &ResolvedSchema) -> Result<String, GenerateError> {
        ...
        for model in &schema.schema.models {
            writeln!(
                output,
                "use super::grpc::{}_grpc_service::{}GrpcService;",
                to_snake_case(&model.name),
                model.name
            )
            .unwrap();
        ...
        writeln!(
            output,
            "    pub fn new(module: &super::{}Module) -> Self {{",
            pascal_name
        )
        .unwrap();
        writeln!(output, "        Self {{").unwrap();
        for model in &schema.schema.models {
            ...
            writeln!(
                output,
                "            {}_service: {}GrpcService::new(module.{}_service.clone()),",
                snake_name, model.name, snake_name
            )
            .unwrap();
        }
        writeln!(output, "        }}").unwrap();
        ...
        for model in &schema.schema.models {
            ...
            writeln!(
                output,
                "            .add_service(self.{}_service.into_service())",
                snake_name
            )
            .unwrap();
        }
    ...
    fn generate_domain_mod(&self, schema: &ResolvedSchema) -> String {
        ...
        if schema
            ...
            .hooks
            .iter()
            .any(|h| !h.computed_fields.is_empty())
        {
...
impl Generator for ModuleGenerator {
    fn generate(&self, schema: &ResolvedSchema) -> Result<GeneratedOutput, GenerateError> {
        ...
        output.add_file(
            PathBuf::from("src/presentation/http/routes/generated.rs"),
            routes_content,
        );
        ...
        if !is_generator_disabled(schema, "grpc") {
            ...
            output.add_file(
                PathBuf::from("src/presentation/grpc/server.rs"),
                grpc_content,
            );
        }
...
mod tests {
    ...
    fn test_module_generator_creates_routes() {
        ...
        assert!(output
            .files
            .contains_key(&PathBuf::from("src/presentation/http/routes/generated.rs")));

        let routes_content = output
            .files
            .get(&PathBuf::from("src/presentation/http/routes/generated.rs"))
            .unwrap();
    ...
    fn test_module_generator_creates_grpc_server() {
        ...
        assert!(output
            .files
            .contains_key(&PathBuf::from("src/presentation/grpc/server.rs")));

        let grpc_content = output
            .files
            .get(&PathBuf::from("src/presentation/grpc/server.rs"))
            .unwrap();
    ...
    fn test_module_generator_creates_layer_mods() {
        ...
        assert!(output
            .files
            .contains_key(&PathBuf::from("src/domain/mod.rs")));
        assert!(output
            .files
            .contains_key(&PathBuf::from("src/infrastructure/mod.rs")));
        assert!(output
            .files
            .contains_key(&PathBuf::from("src/application/mod.rs")));
        assert!(output
            .files
            .contains_key(&PathBuf::from("src/presentation/mod.rs")));
    }
    ...
    fn test_builder_pattern_generated() {
        ...
        assert!(
            lib_content.contains("pub fn readonly_routes(&self)"),
            "a read-only composer must be generated as the safe production base"
        );
        assert!(
            lib_content.contains("// <<< CUSTOM METHODS"),
        ...
        assert!(
            !lib_content.contains("CrudServices"),
            "CrudServices grouping must NOT be emitted"
        );
        assert!(
            !lib_content.contains("crud_services"),
            "crud_services() accessor must NOT be emitted"
        );
        assert!(
            lib_content.contains("pub(crate) user_service: Arc<UserService>"),
