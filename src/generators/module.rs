...
impl ModuleGenerator {
    ...
    fn generate_lib_rs(&self, schema: &ResolvedSchema) -> Result<String, GenerateError> {
        ...
        // Re-exports - Domain entities. Opt-out via config.generators.reexport_entities: false — a
        // module that exposes its contract only through exports::types DTOs + ports omits the glob so
        // sibling crates can't couple to internal entity representations. The omission survives regen
        // (a hand-deletion of the line does not — it is re-emitted every time).
        let reexport_entities = schema
            .schema
            .generators_config
            .as_ref()
            .and_then(|c| c.reexport_entities)
            .unwrap_or(true);
        if reexport_entities {
        ...
        if !hooks_with_validation.is_empty() {
            ...
            writeln!(output, "pub use application::validator::{{ValidationError, ValidationResult}};").unwrap();
            writeln!(output).unwrap();
        }
...
mod tests {
    ...
    fn create_test_schema_with_reexport(reexport: bool) -> ResolvedSchema {
        let mut schema = ModuleSchema::new("orders");
        schema.models.push(create_test_model());
        schema.generators_config = Some(crate::parser::yaml_parser::GeneratorsConfig {
            reexport_entities: Some(reexport),
            ..Default::default()
        });
        ResolvedSchema { schema }
    }
    ...
    #[test]
    fn test_entity_reexport_defaulted_on() {
        // No generators_config → glob present (backward compatible).
        let output = ModuleGenerator::new().generate(&create_test_schema()).unwrap();
        let lib = output.files.get(&PathBuf::from("src/lib.rs")).unwrap();
        assert!(
            lib.contains("pub use domain::entity::*;"),
            "absent flag should default to re-exporting entities, got:\n{lib}"
        );
    }

    #[test]
    fn test_entity_reexport_opt_out_omits_glob() {
        let output = ModuleGenerator::new()
            .generate(&create_test_schema_with_reexport(false))
            .unwrap();
        let lib = output.files.get(&PathBuf::from("src/lib.rs")).unwrap();
        assert!(
            !lib.contains("pub use domain::entity::*;"),
            "reexport_entities: false must omit the entity glob, got:\n{lib}"
        );
    }
