...
impl IntegrationTestGenerator {
    ...
    fn generate_entity_test(
        ...
        entity_paths: &HashMap<String, String>,
    ) -> String {
        let mut output = String::new();
        ...
        for field in &model.fields {
            ...
            let Some(attr) = field.get_attribute("foreign_key") else {
                continue;
            };
        ...
        writeln!(
            output,
            "impl TestDataGenerator for {}TestData {{",
            pascal_name
        )
        .unwrap();
        ...
        for field in &model.fields {
            ...
            if ["created_at", "deleted_at", "created_by", "deleted_by"]
                .contains(&field_name.as_str())
            {
                continue;
            }
        ...
        writeln!(
            output,
            "    inner: GenericCrudTest<{}TestData>,",
            pascal_name
        )
        .unwrap();
        writeln!(output, "}}").unwrap();
        ...
        let supports_soft_delete = model
            .fields
            .iter()
        ...
        writeln!(
            output,
            "        let mut config = CrudTestConfig::new(\"/api/v1/{}\", \"{}\");",
            table_name, pascal_name
        )
        .unwrap();
        writeln!(
            output,
            "        config.supports_soft_delete = {};",
        ...
        writeln!(
            output,
            "            inner: GenericCrudTest::new(config, {}TestData),",
            pascal_name
        )
        .unwrap();
        writeln!(output, "        }}").unwrap();
    ...
    fn generate_integration_tests_main(&self, models: &[&Model], module_name: &str) -> String {
        ...
        writeln!(
            output,
            "//! Run with: cargo test --package backbone-{} --test integration_tests",
            module_name
        )
        .unwrap();
        writeln!(output).unwrap();
...
impl Generator for IntegrationTestGenerator {
    ...
    fn generate(&self, schema: &ResolvedSchema) -> Result<GeneratedOutput, GenerateError> {
        ...
        output.add_file(PathBuf::from("tests/integration/tests/mod.rs"), tests_mod);
        ...
        for model in &models {
            let snake_name = to_snake_case(&model.name);
            let entity_test =
                self.generate_entity_test(model, module_name, &enum_values, &entity_paths);
            output.add_file(
                PathBuf::from(format!(
                    "tests/integration/tests/{}_api_test.rs",
                    snake_name
                )),
                entity_test,
            );
        ...
        output.add_file(PathBuf::from("tests/integration_tests.rs"), main_tests);
