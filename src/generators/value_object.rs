...
use crate::ast::{TypeRef, ValueObject, ValueObjectMethod};
...
impl ValueObjectGenerator {
    ...
    fn generate_value_object(
        ...
        let has_pattern_validation = fields
            .values()
            .any(|f| self.parse_validation_rules(f).pattern.is_some());
        ...
        let all_optional_or_default = fields
            .iter()
            .all(|(_, f)| self.is_field_optional(f) || self.has_field_default(f));
        ...
        if has_validation {
            ...
            writeln!(
                output,
                "    /// Returns Ok(()) if valid, Err with list of validation errors otherwise."
            )
            .unwrap();
            writeln!(
                output,
            ...
            writeln!(
                output,
                "    /// Returns None if valid, Some with errors otherwise."
            )
            .unwrap();
            writeln!(
                output,
    ...
    fn generate_value_object_from_ast(&self, vo: &ValueObject) -> Result<String, GenerateError> {
        let mut output = String::new();
        let name = &vo.name;
        ...
        // Conditional imports based on actual field types
        let needs_datetime = vo
            .fields
            .iter()
            .any(|f| self.type_ref_needs_datetime(&f.type_ref));
        let needs_date = vo
            .fields
            .iter()
            .any(|f| self.type_ref_needs_date(&f.type_ref));
        let needs_time = vo
            .fields
            .iter()
            .any(|f| self.type_ref_needs_time(&f.type_ref));
        let needs_decimal = vo
            .fields
            .iter()
            .any(|f| self.type_ref_needs_decimal(&f.type_ref));
        let needs_uuid = vo
            .fields
            .iter()
            .any(|f| self.type_ref_needs_uuid(&f.type_ref));
    ...
    fn generate_simple_vo_from_ast(
        ...
        if let Some(ref validation) = vo.validation {
            ...
            writeln!(
                output,
                "    pub fn try_new(value: {}) -> Result<Self, String> {{",
                inner_type
            )
            .unwrap();
    ...
    fn generate_vo_method(
        ...
        writeln!(
            output,
            "    pub fn {}({}{}){}{{",
            method.name, self_ref, param_str, return_type
        )
        .unwrap();
    ...
    fn generate_builder(
        ...
        if has_validation {
            ...
            writeln!(
                output,
                "    /// Validates the value object after construction."
            )
            .unwrap();
        }
        writeln!(
        ...
        if has_validation {
            ...
            writeln!(
                output,
                "        value.validate().map_err(|errs| errs.join(\"; \"))?;"
            )
            .unwrap();
            writeln!(output).unwrap();
        }
        ...
        if has_validation {
            writeln!(
                output,
                "    /// Build the {} value object without validation",
                name
            )
            .unwrap();
            writeln!(output, "    ///").unwrap();
            writeln!(
                output,
                "    /// Use with caution - skips all validation rules."
            )
            .unwrap();
            writeln!(
                output,
                "    pub fn build_unchecked(self) -> Result<{}, String> {{",
                name
            )
            .unwrap();
    ...
    fn type_ref_needs_datetime(&self, type_ref: &crate::ast::TypeRef) -> bool {
        ...
        match type_ref {
            TypeRef::Primitive(p) => {
                matches!(p, PrimitiveType::DateTime | PrimitiveType::Timestamp)
            }
...
impl Generator for ValueObjectGenerator {
    fn generate(&self, schema: &ResolvedSchema) -> Result<GeneratedOutput, GenerateError> {
        ...
        for (type_name, fields) in &schema.schema.shared_types {
            ...
            let vo_content = if type_name == "Metadata"
                && fields.len() == 1
        ...
        output.add_file(
            PathBuf::from("src/domain/value_objects/mod.rs"),
            mod_content,
        );
...
mod tests {
    ...
    fn create_test_schema() -> ResolvedSchema {
        ...
        timestamps.insert(
            ...
            YamlField::Simple("datetime".to_string()),
        );
        timestamps.insert(
        ...
        schema
            .shared_types
            .insert("Timestamps".to_string(), timestamps);
    ...
    fn test_value_object_generator_creates_files() {
        ...
        assert!(output
            .files
            .contains_key(&PathBuf::from("src/domain/value_objects/timestamps.rs")));
        assert!(output
            .files
            .contains_key(&PathBuf::from("src/domain/value_objects/mod.rs")));
    }
    ...
    fn test_value_object_has_builder() {
        ...
        let vo_content = output
            .files
            .get(&PathBuf::from("src/domain/value_objects/timestamps.rs"))
            .unwrap();
    ...
    fn test_value_object_has_validation() {
        ...
        let vo_content = output
            .files
            .get(&PathBuf::from("src/domain/value_objects/contact.rs"))
            .unwrap();
