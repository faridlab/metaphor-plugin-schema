...
impl AuditTriggersGenerator {
    ...
    fn generate_audit_triggers_migration(
        &self,
        ...
        writeln!(
            output,
            "-- Audit Triggers Migration for {} module",
            module_name
        )
        .unwrap();
...
mod tests {
    ...
    fn test_audit_triggers_generator_creates_migration() {
        ...
        assert!(output.files.contains_key(&PathBuf::from(
            "migrations/20260426220004_add_audit_triggers.up.sql"
        )));
    }
    ...
    fn test_audit_triggers_schema_qualified_table_and_function() {
        ...
        let content = output
            .files
            .get(&PathBuf::from(
                "migrations/20260426220003_add_audit_triggers.up.sql",
            ))
            .unwrap();

        // Function lives in the table's schema (no public collision).
        assert!(
            content.contains("CREATE OR REPLACE FUNCTION sapiens.docs_audit_timestamp()"),
            "got:\n{}",
            content
        );
        assert!(
            content.contains("EXECUTE FUNCTION sapiens.docs_audit_timestamp()"),
            "got:\n{}",
            content
        );
        // Trigger names stay bare; ON target is qualified.
        assert!(
            content.contains("CREATE TRIGGER docs_insert_audit BEFORE INSERT ON sapiens.docs"),
            "got:\n{}",
            content
        );
        assert!(
            content.contains("CREATE TRIGGER docs_update_audit BEFORE UPDATE ON sapiens.docs"),
            "got:\n{}",
            content
        );
        // No bare, unqualified function/ON references leak through.
        assert!(
            !content.contains("FUNCTION docs_audit_timestamp()"),
            "function must be schema-qualified, got:\n{}",
            content
        );
        assert!(
            !content.contains("ON docs;"),
            "trigger ON target must be qualified, got:\n{}",
            content
        );
    }
