...
impl SqlGenerator {
    ...
    pub fn generate_rls_migration(model: &Model) -> Option<(String, String)> {
        ...
        writeln!(up, "-- Migration: company row-level-security fence for {}", qualified).unwrap();
        writeln!(up, "-- Generated by metaphor-schema (ADR-0008). company_id is scoped per request").unwrap();
        writeln!(up, "-- via `set_config('app.company_id', <uuid>, true)`; an unset var sees zero rows.").unwrap();
        writeln!(up).unwrap();
        writeln!(up, "ALTER TABLE {} ENABLE ROW LEVEL SECURITY;", qualified).unwrap();
        writeln!(up, "ALTER TABLE {} FORCE  ROW LEVEL SECURITY;", qualified).unwrap();
        writeln!(up, "DROP POLICY IF EXISTS {} ON {};", policy, qualified).unwrap();
        writeln!(up, "CREATE POLICY {} ON {}", policy, qualified).unwrap();
        writeln!(up, "    FOR ALL").unwrap();
        writeln!(up, "    USING      (company_id = {})", fence).unwrap();
        writeln!(up, "    WITH CHECK (company_id = {});", fence).unwrap();

        let mut down = String::new();
        writeln!(down, "-- Reverse the company RLS fence for {}", qualified).unwrap();
        writeln!(down, "DROP POLICY IF EXISTS {} ON {};", policy, qualified).unwrap();
        writeln!(down, "ALTER TABLE {} NO FORCE ROW LEVEL SECURITY;", qualified).unwrap();
        writeln!(down, "ALTER TABLE {} DISABLE ROW LEVEL SECURITY;", qualified).unwrap();
...
impl SqlGenerator {
    ...
    fn generate_split(&self, output: &mut GeneratedOutput, schema: &ResolvedSchema) -> Result<(), GenerateError> {
        ...
        for &model_idx in &order {
            ...
            // migration_cleanup sweep only scans the top ~10 lines for the
            // marker, and an enum-heavy prelude can otherwise push it past
            // that window, causing stale renamed files to be preserved as
