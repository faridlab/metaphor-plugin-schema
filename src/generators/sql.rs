...
impl SqlGenerator {
    ...
    fn generate_table(&self, model: &Model) -> Result<String, GenerateError> {
        ...
        // Bare name for derived identifiers (index/trigger names — Postgres scopes
        // these to the table's schema); qualified name for table references.
        let table_name = model.collection_name();
        let qualified = model.qualified_table_name();

        // Header comment
        writeln!(output, "-- Migration: Create {} table", qualified).unwrap();
        writeln!(output, "-- Generated by metaphor-schema").unwrap();
        writeln!(output).unwrap();

        // Ensure the target schema exists before creating the table in it.
        if let Some(s) = model.schema.as_deref().filter(|s| !s.is_empty()) {
            writeln!(output, "CREATE SCHEMA IF NOT EXISTS {};", s).unwrap();
            writeln!(output).unwrap();
        }

        // Create table
        writeln!(output, "CREATE TABLE IF NOT EXISTS {} (", qualified).unwrap();
        ...
        for index in &model.indexes {
            writeln!(output).unwrap();
            let index_sql = self.generate_index(&table_name, &qualified, index, model)?;
        ...
        for field in &model.fields {
            if field.has_attribute("audit_metadata") {
                ...
                writeln!(output).unwrap();
                self.generate_audit_triggers(&mut output, model, field)?;
            }
        }
    ...
    /// The `company_id` column that fences this model, or `None` if the model is not
    /// company-scoped.
    ///
    /// Mirrors the Rust generator's `company_field()` polarity exactly (rust.rs): a column
    /// literally named `company_id` fences the table **unless** it carries `@global`. A model
    /// with no such column (a child row like `SalesInvoiceItem`, or true reference data) is not
    /// fenced. The polarity is opt-*out*: forgetting leaves the table fenced; only a deliberate
    /// `@global` in a diff unfences it.
    fn company_fence_column(model: &Model) -> Option<&str> {
        model
            .fields
            .iter()
            .find(|f| f.name == "company_id" && !f.has_attribute("global"))
            .map(|f| f.name.as_str())
    }

    /// Emit the Row-Level-Security migration for one model, or `None` when the model is not
    ...
    ///
    /// - `FORCE ROW LEVEL SECURITY` — a merely-`ENABLE`d policy is bypassed by the table owner;
    ///   `FORCE` fences the owner too. The request role must additionally be a non-superuser
    ///   (superusers always bypass RLS); migrations/seeders run as the owner/superuser and are the
    ///   deliberately-unfenced system path.
    /// - `NULLIF(current_setting('app.company_id', true), '')::uuid` — the `true` makes an unset
    ///   var return NULL rather than erroring, and `NULLIF(..,'')` handles the case where a prior
    ///   `set_config(..,true)` left the session var as `''` (empty string): both collapse to NULL,
    ...
    pub fn generate_rls_migration(model: &Model) -> Option<(String, String)> {
        ...
        up.insert_str(
            0,
            &format!(
                "-- Migration: company row-level-security fence for {qualified}\n\
    ...
    fn generate_audit_triggers(&self, output: &mut String, model: &Model, field: &Field) -> Result<(), GenerateError> {
        ...
        // don't collide on one `public` function. Trigger *names* stay bare —
        // Postgres scopes them to their table.
    ...
    fn generate_foreign_keys(&self, model: &Model, schema: &ResolvedSchema) -> Result<String, GenerateError> {
        ...
        for relation in &model.relations {
            ...
            let target_name = get_relation_target_table_qualified(&relation.target, &schema.schema);
            let references = relation.references();

            // Build constraint name (bare table — constraints are scoped to their table)
...
impl SqlGenerator {
    ...
    fn generate_split(&self, output: &mut GeneratedOutput, schema: &ResolvedSchema) -> Result<(), GenerateError> {
        ...
        for &model_idx in &order {
            ...
            // Emit the `-- Generated by metaphor-schema` marker first so it
            // always lives within the first few lines of the file — the
            // `is_generator_authored_migration` check scans the top ~10 lines
            // for the marker, and an enum-heavy prelude can otherwise push it
            // past that window.
            let mut up = String::new();
            ...
            // `generate_table` repeats its own header — strip it to avoid
            // a duplicate `-- Migration:` block now that we've emitted one
            // up front.
            let table_sql = self.generate_table(model)?;
            up.push_str(strip_generated_header(&table_sql));
        ...
        if !rls.is_empty() {
            ...
            output.add_file(
                PathBuf::from(format!("migrations/{}_enable_company_rls.up.sql", ts)),
                up,
            );
            output.add_file(
                PathBuf::from(format!("migrations/{}_enable_company_rls.down.sql", ts)),
                down,
            );
        }
...
/// blank` header that `generate_table` emits. Used when we've already
/// emitted that header at the very top of the file (before the enum
/// prelude) so the marker stays inside the sweep's scan window.
fn strip_generated_header(sql: &str) -> &str {
    let mut iter = sql.char_indices();
    // Skip up to 3 leading header lines.
    let mut consumed_end = 0;
    for _ in 0..3 {
        let Some((idx, _)) = iter.find(|(_, c)| *c == '\n') else {
            break;
        };
        let line_start = consumed_end;
        let line = &sql[line_start..idx];
        let trimmed = line.trim_start();
        if trimmed.starts_with("-- Migration:")
            || trimmed.starts_with("-- Generated by metaphor-schema")
            || trimmed.is_empty()
        {
            consumed_end = idx + 1;
            continue;
        }
        // First non-header line — stop stripping.
        break;
    }
    &sql[consumed_end..]
}
...
/// Resolve a relation's target table for use as an FK `REFERENCES` target,
/// schema-qualifying same-module (`Custom`) targets with the target model's own
/// schema (so a FK into a schema-scoped table emits `schema.table`).
///
...
/// Resolve a relation's target table using the **bare** collection name for
/// same-module targets (no schema qualification).
fn get_relation_target_table(type_ref: &TypeRef, schema: &crate::ast::ModuleSchema) -> String {
    resolve_relation_target_table(type_ref, schema, false)
}

/// Shared resolver for [`get_relation_target_table`] and its qualified variant.
/// When `qualify` is set, a same-module (`Custom`) target uses the target
/// model's `schema.table`; otherwise the bare collection name. Only the `Custom`
/// arm differs — every other arm resolves identically.
fn resolve_relation_target_table(
    type_ref: &TypeRef,
    ...
    match type_ref {
        ...
        TypeRef::Optional(inner) => resolve_relation_target_table(inner, schema, qualify),
...
mod tests {
    ...
    /// A company-scoped model in a named schema, mirroring a real fenced entity.
    fn company_scoped_model() -> Model {
    ...
    #[test]
    fn generate_split_emits_the_rls_migration_as_the_last_file() {
        ...
        // The RLS migration exists as its own additive file...
        let rls_up = output
            .files
            .keys()
        ...
        // ...and its timestamp sorts AFTER the create-table migration, so the table exists first.
        let table_up = output
            .files
            .keys()
        ...
        assert!(
            ...
            "RLS migration must run after the table is created"
        );
    }
    ...
    // ── Per-module schema scoping ────────────────────────────────────────────

    #[test]
    fn test_qualified_table_and_audit_fn_helpers() {
        let mut m = Model::new("NotificationPreference");
    ...
    #[test]
    fn test_schema_scoped_table_emits_create_schema_and_qualifies_refs() {
    ...
    #[test]
    fn test_audit_function_qualified_into_schema_trigger_name_bare() {
    ...
    #[test]
    fn test_same_module_fk_target_is_schema_qualified() {
        // Post (unscoped) FK -> User (schema `identity`): REFERENCES must be
        ...
        let mut user = create_test_model();
        user.schema = Some("identity".to_string());
