...
fn is_generator_authored_migration(path: &Path) -> bool {
...
fn insert_custom_blocks(result_lines: &mut Vec<String>, custom_blocks: &[(Option<String>, Vec<String>)]) {
    for (anchor, block_lines) in custom_blocks {
        ...
        if let Some(pos) = insert_pos {
            ...
            // fields, use-list) or a SIBLING at module/function scope. The
            // anchor alone is ambiguous — `pub b: i32,` (struct field) and
            // `B,` (last item in `pub use foo::{ A, B, };`) both end with `,`
            // but the CUSTOM block belongs in different places.
            //
            ...
            //     the block is module-scope → walk past close braces so it
            //     lands AFTER any containing `};`.
            //   - Otherwise the block looks field-like (identifier comma,
            //     variant comma) → keep position so it stays INSIDE the
            //     enclosing container.
            let first_content = block_lines.iter()
                .find(|l| !is_whole_line_custom_marker(l)
                    && !l.trim().is_empty()
