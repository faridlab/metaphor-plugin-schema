...
//! 3. **`exists()` gate** — skip if the file is on disk and `--force` was
//!    not passed. (Migration identity/timestamp stability is handled once,
//!    upstream, by `stabilize_migration_timestamps` — this phase is a dumb
//!    writer.)
...
pub(super) fn write_generated_files(
    ...
    for (path, content) in &generated.files {
        ...
        if dry_run {
            ...
            // (3) exists() gate. Migration identity/timestamp stability was
            // already resolved upstream by `stabilize_migration_timestamps`,
            // so a stabilized existing-identity path already points at its
            // on-disk filename and is skipped here under `!force`.
